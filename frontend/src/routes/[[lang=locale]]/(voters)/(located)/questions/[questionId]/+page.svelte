...
<script lang="ts">
  ...
  let useQuestionOrdering = ($appSettings.questions.dynamicOrdering?.enabled ?? false)
    && ($appSettings.questions.dynamicOrdering?.config?.type === 'factor-based');
  let nextQuestionChoices: Array<AnyQuestionVariant> = [];
  let lastShownQuestionIds: Array<string> = [];
  let cachedChoices: Array<AnyQuestionVariant> = [];
  ...
  $: shouldShowQuestionSelection = useQuestionOrdering && $page.url.searchParams.get('showQuestionSelection') === 'true';
  ...
  onMount(() => {
    ...
    if (shouldShowQuestionSelection && numSuggestions === 1) {
      ...
      const nextQuestion = getNextQuestion(shownIds, allQuestions);
  ...
  // Helper function to get a single next question
  function getNextQuestion(shownIds: Array<string>, allQuestions: Array<AnyQuestionVariant>): AnyQuestionVariant | undefined {
    ...
    // If we have factor loadings, use them for intelligent question ordering
    if ($factorLoadings) {
      const orderer = new QuestionOrderer(allQuestions, $factorLoadings);
      const suggestions = orderer.getNextQuestions(shownIds, 1);
      if (suggestions.length > 0) {
        return suggestions[0];
      }
    }

    return unshownQuestions[Math.floor(Math.random() * unshownQuestions.length)];
  }
  ...
  function getNextQuestionChoices(): Array<AnyQuestionVariant> {
    ...
    // Check if we've already calculated for these exact shown questions
    if (
      JSON.stringify(shownIds) === JSON.stringify(lastShownQuestionIds) &&
      cachedChoices.length > 0
    ) {
      return cachedChoices;
    }

    // Store the current shown IDs for future comparison
    lastShownQuestionIds = [...shownIds];
    ...
    if ($factorLoadings) {
      const orderer = new QuestionOrderer(allQuestions, $factorLoadings);
      const suggestions = orderer.getNextQuestions(shownIds, numSuggestions);
      if (suggestions.length > 0) {
        cachedChoices = suggestions;
        return suggestions;
      }
    }

    // Fallback: return random unshown questions if no factor loadings or if ordering fails
    cachedChoices = unshownQuestions
  ...
  function handleJumpForQuestionOrdering(steps: number): void {
    ...
    if (newIndex < 0) {
      ...
      // If we don't already have choices calculated, get them now
