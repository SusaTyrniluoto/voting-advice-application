  ...
  for (const electionId in tree) {
    for (const electionRound in tree[electionId]) {
      for (const constituencyId in tree[electionId][electionRound]) {
        ...
        const allianceNominations: Array<PublicAllianceNominationData> = [...alliances.values()].map(
          ({ organizations: allianceOrganizations, ...rest }) => {
            const allies = allianceOrganizations.map((id) => {
              const index = orgNominations.findIndex((o) => o.entityId === id);
              if (index < 0)
                throw new Error(`Allied organization with id ${id} not found or assigned to multiple Alliances`);
